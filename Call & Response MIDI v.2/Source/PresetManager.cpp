...
bool PresetManager::loadFromSlot(int slotIndex, StateSnapshot& outSnapshot) {
...
void PresetManager::pushUndo(const UndoStep& step) {
    if (!undoStack.empty()) {
        const auto& last = undoStack.back();
        if (last.call == step.call && last.response == step.response)
            return; // Avoid pushing duplicate identical states
    }
...
UndoStep PresetManager::undo(const UndoStep& current) {
...
UndoStep PresetManager::redo(const UndoStep& current) {
...
juce::ValueTree PresetManager::snapshotToValueTree(const StateSnapshot& snap, const juce::String& name) {
    ...
    cfgVt.setProperty("baseVelocityIndex", snap.config.baseVelocityIndex, nullptr);
    cfgVt.setProperty("baseVelocity", snap.config.baseVelocity, nullptr);
    cfgVt.setProperty("accentAmountIndex", snap.config.accentAmountIndex, nullptr);
    cfgVt.setProperty("variationIndex", snap.config.variationIndex, nullptr);
    ...
    lockVt.setProperty("lockCall", snap.lockCall, nullptr);
    lockVt.setProperty("lockResponse", snap.lockResponse, nullptr);
...
StateSnapshot PresetManager::valueTreeToSnapshot(const juce::ValueTree& vt) {
    ...
    if (cfgVt.isValid()) {
        ...
        snap.config.baseVelocityIndex = cfgVt.getProperty("baseVelocityIndex", snap.config.baseVelocityIndex);
        snap.config.baseVelocity = cfgVt.getProperty("baseVelocity", snap.config.baseVelocity);
        snap.config.accentAmountIndex = cfgVt.getProperty("accentAmountIndex", snap.config.accentAmountIndex);
        snap.config.variationIndex = cfgVt.getProperty("variationIndex", snap.config.variationIndex);
    ...
    if (lockVt.isValid()) {
        ...
        snap.lockCall = lockVt.getProperty("lockCall", false);
        snap.lockResponse = lockVt.getProperty("lockResponse", false);
